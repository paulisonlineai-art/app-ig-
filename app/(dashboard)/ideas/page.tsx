...
export default async function IdeasPage() {
  ...
  const [{ data: topReels }, { data: competitors }, { data: brand }] = await Promise.all([
    db.from('reels').select('id,caption,views,multiplier,save_rate,like_rate,comment_rate,hook,transcript').eq('account_id', accountId).order('multiplier', { ascending: false }).limit(20),
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 28, fontWeight: 800, letterSpacing: '-0.03em', marginBottom: 2 }}>Moka AI</h1>
